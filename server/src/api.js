...
async function delistLead(db, lead, on) {
  ...
  // second one deletes nothing. Saying so keeps the run's `delisted` count,
  // which is tallied from these responses, from counting that lead twice.
...
// uses it and an in-flight nightly run may still be part-way through a night on
// the old wording, so it keeps working and keeps its exact response shape -
// `removed` true/false is what a run tallies its `delisted` count from.
