...
- `GET /api/coverage/:key` -> `{ companies: [{company, last_swept, board, note}] }` - which companies this search tracks and when each was last *attempted*, least-recently-swept first (never-swept sorts first of all). A company list long enough to be worth having is longer than one run can verify properly, so a run covers a bounded slice and rotates; this is the memory that stops it starting at the top of the list every night. 404s on an unknown key for the same reason `/api/dedup/:key` does - an empty list reads as "nothing has ever been swept", which would send the run at everything at once.
...
- A track with any coverage rows gets two extra steps in its composed prompt (1c: which companies to cover; 9d: record what you covered). That's gated on the rows existing rather than on a config flag - seeding the table is what turns rotation on for a search, and there's nothing to remember to set.
