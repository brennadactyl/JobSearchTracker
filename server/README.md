...
- `POST /api/update` - body `{ "type": "lead", "id": ..., "status": "...", "notes": "..." }` or `{ "type": "application", ... }`. All fields are optional per call. There is no field here for "this posting has been taken down" - a dead posting is deleted rather than flagged, see `/api/delete-lead` below.
...
- `POST /api/delete-lead` - body `{ "id": ..., "reason": "..." }` - what a scheduled search calls when a posting it tracks is confirmed gone from the internet. Deletes the lead **and** writes its URL into `screened` in one transaction: the delete on its own would just mean tomorrow's run rediscovers the URL, finds nothing tracking it, and adds it straight back as a new lead. `reason` is optional and defaults to "posting taken down". **409s on a lead whose status is "Applied"** (with `"kept": true`) rather than deleting it - an application row points at that id, and once you've applied, what's tracked is the application, not whether the posting outlived it. This is enforced here rather than left to the prompt, because the caller is an LLM working from a list of ids.
