...
8. If a previously-tracked lead is confirmed dead on a later check, **never delete or move it yourself** - report it and let the tracker decide (`POST /api/update`, `delistedOn` = today; procedure is step 8 of the daily prompt), whatever `status` it's in. The server acts on that report rather than storing it: a lead that hasn't been applied to is removed from the board and its URL recorded as screened so the next run doesn't re-add it, and an applied-to lead is kept, because what's tracked there is the application rather than whether the listing survived. So don't post it to `/api/screened` separately either. A removal is permanent - the row is gone and the screened URL is skipped from then on, so a posting reported dead by mistake never comes back on a later run. If a posting couldn't be re-verified due to a fetch/tooling problem (timeout, blocked domain, truncated content) rather than a confirmed 404/closed page, **report nothing** - leave the lead alone and note the tooling issue instead, don't treat "couldn't check" as "dead."
