...
8. If a previously-tracked lead is confirmed dead on a later check, remove it from the tracker (`POST /api/delete-lead` with its id; procedure is step 8 of the daily prompt) - there's nothing left to apply to, so it shouldn't keep taking up room in the search tab. That endpoint deletes the lead and records its URL as screened in one transaction, so the next run doesn't rediscover it and add it back; don't post it to `/api/screened` separately. **Never send a lead whose `status` is `"Applied"`** - the installer has applied to it, what's tracked from then on is the application, and the server refuses those (409) anyway. If a posting couldn't be re-verified due to a fetch/tooling problem (timeout, blocked domain, truncated content) rather than a confirmed 404/closed page, **don't delete anything** - leave the lead alone and note the tooling issue instead, don't treat "couldn't check" as "dead."
