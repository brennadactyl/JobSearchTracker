...
8. Rewrite `docs/tracked_{{TRACK_KEY}}_postings.md` in full: keep existing rows and the dead-link list, append new verified postings with today's date and "Verified live <date>". **Never remove a posting from the found table just because it's gone dead.** Instead, for a previously-listed posting confirmed dead on this check, use the step-1b tracker data (match by `url`) to call `POST /api/update` marking it delisted:
   ```
