...
1b. Fetch the tracker's current lead data: `curl -s "$TRACKER_URL/api/data" -H "Authorization: Bearer $TRACKER_API_TOKEN"`. Keep each lead's `url` and `status` on hand for step 8 - it's how you tell a stale lead nobody's touched from one the installer has already applied to.
...
8. Rewrite `docs/tracked_{{TRACK_KEY}}_postings.md` in full: keep existing rows and the dead-link list, append new verified postings with today's date and "Verified live <date>". For a previously-listed posting confirmed dead on this check, use the step-1b tracker data to decide (match by `url`): **status still `"New"`** - move it to "Removed / went dead", it's a stale lead nobody's followed up on. **Status is anything else** (Applied, Interviewing, Rejected, etc.) - leave it in the found table, don't remove it, just note the original posting is no longer live as of today; the installer still needs to track it regardless of whether the listing survives. If the tracker's unreachable or the URL isn't found there, treat it the same as "status changed" - leave it in place rather than risk losing track of a real application. This doc is the durable source of truth for postings.
