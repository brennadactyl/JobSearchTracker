...
.PARAMETER Task
  ...
  One value isn't a track: "_applications" fetches the nightly fill for
  applications logged as nothing but a URL (see ../server/src/prompt.js's
  buildAutofillPrompt). It's a reserved key on the same /api/prompt route,
  which is the whole reason it needs no special handling here -
  underscore-first so it can't collide with a track someone actually named.
